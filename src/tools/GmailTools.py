...
class GmailToolsClass:
    ...
    def fetch_unanswered_emails(self, max_results=50):
        ...
        try:
            ...
            raise RuntimeError("Could not load Gmail messages") from e
    ...
    def fetch_recent_emails(self, max_results=50):
        try:
            ...
            query = f"in:inbox -in:drafts after:{after_timestamp} before:{before_timestamp}"
            ...
            raise RuntimeError("Could not query the Gmail inbox") from error
    ...
    def fetch_draft_replies(self):
        ...
        try:
            ...
            raise RuntimeError("Could not check existing Gmail drafts") from error
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                if not credentials_path.exists():
                    raise RuntimeError('Download Desktop app OAuth credentials to credentials.json first.')
                config = json.loads(credentials_path.read_text(encoding='utf-8-sig'))
                if 'installed' not in config:
                    raise RuntimeError('credentials.json must be a Desktop app OAuth client. Create one in Google Cloud and download its JSON.')
