...
from .config import KNOWLEDGE_MODE, SAMPLE_NOTICE
...
class Nodes:
    ...
    def must_rewrite(self, state: GraphState) -> str:
        ...
        if email_sendable:
            print(Fore.GREEN + "Email passed review; ready to create a draft." + Style.RESET_ALL)
            return "send"
        elif state["trials"] >= 3:
    ...
    def create_draft_response(self, state: GraphState) -> GraphState:
        ...
        if KNOWLEDGE_MODE == "sample":
            reply_text = SAMPLE_NOTICE + "\n\n" + reply_text
        draft = self.gmail_tools.create_draft_reply(state["current_email"], reply_text)
        if not draft or not draft.get("id"):
            raise RuntimeError("Gmail did not confirm draft creation; stopping this run.")
        
        return self.finish_email(state)
    ...
    def skip_unrelated_email(self, state):
        ...
        return self.finish_email(state)
    def finish_email(self, state: GraphState) -> GraphState:
        """Commit queue advancement and clear all per-email state."""
        return {
            "emails": state["emails"][:-1], "writer_messages": [],
            "retrieved_documents": "", "rag_queries": [], "trials": 0,
            "generated_email": "", "sendable": False,
        }
